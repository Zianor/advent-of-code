...
data = np.array(open(get_data(10), 'r').read().split('\n'), dtype=np.int)
"""
...
product = three_jolt_differences * one_jolt_differences

print(f"The number of 1-jolt differences multiplied by the number of 3-jolt differences is {product}")
...
def get_possibilities(joltages):
    possibilities = 1
    current_joltage = 0
    while current_joltage != joltages.max():
        next_joltages = joltages[np.logical_and(joltages > current_joltage, joltages <= current_joltage + 3)]
        if joltages[joltages > current_joltage + 3].any() and next_joltages.any():
            possibilities *= investigate_possibilities(joltages, current_joltage,
                                                       joltages[joltages > current_joltage + 3].min())
        current_joltage = np.max(next_joltages)
    return possibilities


def investigate_possibilities(joltages, start_joltage, target_joltage):
    next_joltages = joltages[np.logical_and(joltages > start_joltage, joltages <= start_joltage + 3)]
    if len(next_joltages) == 0 or next_joltages.min() >= target_joltage:
        return 1
    else:
        sum = 0
        for joltage in next_joltages:
            sum += investigate_possibilities(joltages, start_joltage=joltage, target_joltage=target_joltage)
        return sum


print(f"The number of distinct ways to arrange the adapters is {get_possibilities(data)}")
