...
data = open(get_data(8), 'r').read().split('\n')
"""
...
def prepare_instructions(instructions):
    return [{'cmd': instruction.split(' ')[0], 'number': int(instruction[instruction.index(' ') + 1:], ),
            'visited': False} for instruction in instructions]


def run_instruction(instruction, pos, acc):
    if instruction['cmd'] == 'jmp':
        pos = pos + instruction['number']
    else:
...
def run_boot_code(instructions):
    ...
    while pos != len(instructions):
        ...
        pos, acc = run_instruction(instructions[pos], pos, acc)
    return acc, True


print(f"The value of the accumulator is {run_boot_code(prepare_instructions(data))[0]}")
...
def fix_boot_code(data):
    instructions = prepare_instructions(data)
    for i, instruction in enumerate(instructions):
        if instruction['cmd'] == 'jmp':
            instructions[i]['cmd'] = 'nop'
        elif instruction['cmd'] == 'nop':
            instructions[i]['cmd'] == 'jmp'
        else:
            continue
        acc, success = run_boot_code(instructions)
        if success:
            return acc
        else:
            instructions = prepare_instructions(data)
...
print(f"The value of the accumulator for the fixed program is {fix_boot_code(data)}")
