...
data = open(get_data(12), 'r').read().split('\n')
"""
--- Day 12: Rain Risk ---
...
def get_manhatten_distance(instructions):
    ...
    for instruction in instructions:
        ...
        if action == 'F':
            pos = (pos[0] + number * directions[facing][0], pos[1] + number * directions[facing][1])
        elif action == 'L':
            facing = (facing - number // 90) % (len(directions))
        elif action == 'R':
            facing = (facing + (number // 90)) % (len(directions))
        else:
            pos = move(action, number, pos)
    return abs(pos[0]) + abs(pos[1])


print(f"The manhatten distance is {get_manhatten_distance(data)}")
...
N3 moves the waypoint 3 units north to 10 units east and 4 units north of the ship. The ship remains at east 100, north 
10.
...
def get_manhatten_distance_waypoint(instructions):
    pos = (0, 0)
    waypoint = (10, 1)
    for instruction in instructions:
        action = instruction[0]
        ...
        if action == 'F':
            ...
            waypoint = move(action, number, waypoint)
    return abs(pos[0]) + abs(pos[1])


print(f"The manhatten distance is actually {get_manhatten_distance_waypoint(data)}")
