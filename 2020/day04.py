...
passports = np.array(open(get_data(4), 'r').read().split('\n\n'))


def valid_part1(passport):
    passport = passport.replace('\n', ' ')
    areas = passport.split(' ')
    if len(areas) == 8:
        return True
    elif len(areas) == 7:
        for area in areas:
...
valid_passports = np.array([valid_part1(passport) for passport in passports], dtype=np.bool).sum()
...
def check_heigth(hgt):
    if hgt.endswith("in"):
        number = int(hgt.replace("in", ""))
        return 59 <= number <= 76
    elif hgt.endswith("cm"):
        number = int(hgt.replace("cm", ""))
        return 150 <= number <= 193
    return False


def check_hair_color(area):
    if area.startswith('#') and len(area) == 7:
        if re.match('[0-9a-f]{6}', area.replace('#', '')):
            return True
    return False
...
def valid_part2(passport):
    field_validation = {
        ...
        'hgt': lambda x: check_heigth(x),
        'eyr': lambda x: 2020 <= int(x) <= 2030,
        'iyr': lambda x: 2010 <= int(x) <= 2020,
        'hcl': lambda x: check_hair_color(x),
        'ecl': lambda x: x in ['amb', 'blu', 'brn', 'gry', 'grn', 'hzl', 'oth'],
        'pid': lambda x: re.match('[0-9]{9}', x) is not None and len(x) == 9,
        'cid': lambda x: True
    }
    passport = passport.replace('\n', ' ')
    areas = passport.split(' ')
    for area in areas:
        print(area, field_validation[area.split(':')[0]](area.split(':')[1]))
        if not field_validation[area.split(':')[0]](area.split(':')[1]):
            return False
    return True


valid_passports_validation = np.array([valid_part1(passport) and valid_part2(passport) for passport in passports],
                                      dtype=np.bool).sum()

print(f"There are {valid_passports_validation} valid passports with input validation")
