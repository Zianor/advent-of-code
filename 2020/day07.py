...
def get_bags(bag_searched):
    bag_rules = [bag_rule.split('s contain ')[0] for bag_rule in data if bag_searched in bag_rule.split('contain')[1]]
    bags = set()
    for bag in bag_rules:
        bags.add(bag)
        bags_found = get_bags(bag)
        if bags_found:
            bags.update(bags_found)
    return bags


print(f"{len(get_bags('shiny gold bag'))} bag colors can contain shiny gold bags")
...
def get_required_bag_number(bag_searched):
    bag_rule = [bag_rule.split('s contain ')[1] for bag_rule in data if bag_searched in bag_rule.split('contain')[0]]
    if not bag_rule:
        return 0
    number = 0
    for bag in bag_rule[0].split(', '):
        count = bag.split(' ')[0]
        if count.isnumeric():
            count = int(count)
            number += count + count * get_required_bag_number(bag.split(' ')[1] + ' ' + bag.split(' ')[2])
    return number


print(f"There are {get_required_bag_number('shiny gold bag')} bags required in your single shiny gold bag")
