...
def get_ith_number(starting_numbers, turn_number):
    last_seen = {}

    for i, number in enumerate(starting_numbers):
        previous = number
        last_seen[number] = i + 1

    i += 1  # 0 based indices to 1 based

    while i < turn_number:

        if previous not in last_seen:
            curr = 0
        else:
            curr = i - last_seen[previous]
...
print(f"The 2020th number is {get_ith_number(starting_numbers=data, turn_number=2020)}")
...
print(f"The 30000000th number is {get_ith_number(starting_numbers=data, turn_number=30000000)}")
