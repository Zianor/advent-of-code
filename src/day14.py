...
data = open(get_data(14), 'r').read().split('\n')
...
for line in data:
    if line.startswith("mask"):
        ...
        binary_number = bin(addition).replace("0b", "").zfill(36)
        for i, mask_value in enumerate(mask):
            if mask_value != 'X':
                binary_number = binary_number[:i] + mask_value + binary_number[i + 1:]
        memory[pos] = int(binary_number, base=2)

print(f"The memory sum is {sum(memory.values())}")
...
for line in data:
    if line.startswith("mask"):
        ...
        pos = int(line.split("[")[1].split("]")[0])
        addition = int(line.split(" = ")[1])
        binary_position = bin(pos).replace("0b", "").zfill(36)
        floating_bits = []
        for i, mask_value in enumerate(mask):
            if mask_value != '0':
                binary_position = binary_position[:i] + mask_value + binary_position[i + 1:]
                if mask_value == 'X':
                    floating_bits.append(2**(35 - i))
        positions = [int(binary_position.replace('X', "0"), base=2)]
        memory[int(binary_position.replace('X', "0"), base=2)] = addition
        for bit in floating_bits:
            copied_list = positions[:]
            for position in copied_list:
                positions.append(position + bit)
                memory[position + bit] = addition

print(f"The memory sum is now {sum(memory.values())}")
