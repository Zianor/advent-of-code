...
data = np.array(open(get_data(1), 'r').read().split('\n'), dtype=np.int64)
...
sums = data + data.reshape(-1, 1)
position = np.argwhere(sums == 2020)[0]

product = data[position[0]] * data[position[1]]
print(f"The numbers are {data[position[0]]} and {data[position[1]]} and their product is {product}")
...
"""
sums_from_3 = sums.flatten()[0:20000] + data.reshape(-1, 1)

position_from_3 = np.argwhere(sums_from_3 == 2020)

positions_from_3_indices = np.unique(position_from_3[:, 0])

product_from_3 = data[positions_from_3_indices[0]] * data[positions_from_3_indices[1]] * data[
    positions_from_3_indices[2]]

print(
    f"The numbers are {data[positions_from_3_indices[0]]}, {data[positions_from_3_indices[1]]} and "
    f"{data[positions_from_3_indices[2]]} and their product is {product_from_3}")
