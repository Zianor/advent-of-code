...
data = open(get_data(6), 'r').read().split('\n\n')
...
"""
counts = np.array([len(set(group_data.replace('\n', ''))) for group_data in data])

print(f"The sum of those counts is {counts.sum()}")
...
def get_group_count_agreed(group_data):
    answers = group_data.split('\n')
    count = 0
    for question in set(group_data.replace('\n', '')):
        exists = True
...
counts_agreed = np.array([get_group_count_agreed(group_data) for group_data in data])

print(f"The sum of the agreed counts is {counts_agreed.sum()}")
