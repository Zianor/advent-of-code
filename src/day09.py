...
data = np.array(open(get_data(9), 'r').read().split('\n'), dtype=np.int)
"""
...
def is_valid(numbers, value):
    sums = numbers + numbers.reshape(-1, 1)
    return value in sums


def first_invalid_number(data):
    for i, value in enumerate(data[25:]):
        if not is_valid(data[i:i + 25], value):
            return value


invalid_number = first_invalid_number(data)

print(f"The first invalid number is {invalid_number}")
...
def get_encryption_weakness(data, invalid_number):
    start = 0
    end = 1
    while data[start:end].sum() != invalid_number:
        if data[start:end].sum() < invalid_number:
            end += 1
        else:
            start += 1
    return data[start:end].min() + data[start:end].max()


print(f"The encryption weakness is {get_encryption_weakness(data, invalid_number)}")
