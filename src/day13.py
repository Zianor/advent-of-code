...
data = open(get_data(13), 'r').read().split('\n')
...
for bus in busses:
    if bus != 'x':
        bus_id = int(bus)
        leaves_in = (((timestamp - 1) // bus_id) + 1) * bus_id - timestamp
        if leaves_in < min_waiting_time:
            min_waiting_time = leaves_in
            best_bus = bus_id


print(f"The best bus is {best_bus} and the product is {best_bus * min_waiting_time}")
...
def get_first_timestamp(busses):
    time = 1
    step_size = 1
...
print(f"The first timestamp is {get_first_timestamp(busses)}")
