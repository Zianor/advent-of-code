...
data = np.array(open(get_data(3), 'r').read().split('\n'))
...
def get_trees(right, down):
    trees = np.array([line[np.mod(i * right, len(line))] == '#' for i, line in enumerate(data[::down])], dtype=np.bool)
    return trees.sum()


print(f"You would encounter {get_trees(3, 1)} trees")
