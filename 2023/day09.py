...
zeros, repeat this process, using the sequence you just generated as the input sequence. Once all of the values in your
...
def get_sequence_of_differences(sequence):
    new_sequence = []
...
def part1(puzzle_input):
    ...
    for line in puzzle_input:
        ...
        all_zeros = all([val == 0 for val in sequence])
        collected_sequences = []
        while not all_zeros:
            collected_sequences.append(sequence)
            sequence = get_sequence_of_differences(sequence)
            if all([val == 0 for val in sequence]):
                all_zeros = True
        last_value = 0
        for i in range(len(collected_sequences) - 1, -1, -1):
            last_value = collected_sequences[i][-1] + last_value
        next_sequence_values.append(last_value)
    return sum(next_sequence_values)


def part2(puzzle_input):
    previous_sequence_values = []
    for line in puzzle_input:
        sequence = [int(val) for val in line.split()]
        all_zeros = all([val == 0 for val in sequence])
        collected_sequences = []
        ...
        last_value = 0
        for i in range(len(collected_sequences) - 1, -1, -1):
            last_value = collected_sequences[i][0] - last_value
        previous_sequence_values.append(last_value)
    return sum(previous_sequence_values)
...
if __name__ == "__main__":
    ...
    assert res_part1_test == 114
    data = open(get_data(9), "r").read().split("\n")
