...
def get_max_count_for_game_per_color(game):
    comb_strings = game.split(":")[1].split(";")
    max_counts = {
        "blue": 0,
        "green": 0,
        "red": 0,
    ...
    for comb in comb_strings:
        color_strings = comb[1:].split(", ")
        for color_string in color_strings:
            count = int(color_string.split(" ")[0])
...
def part1(data):
    ...
    for game in data:
        game_id_string = game.split(":")[0]
        game_id = int(game_id_string[5:])
        max_counts = get_max_count_for_game_per_color(game)
        if (
            max_counts["blue"] <= 14
            and max_counts["red"] <= 12
...
def part2(data):
    ...
    for game in data:
        max_counts = get_max_count_for_game_per_color(game)
        curr_value = max_counts["blue"] * max_counts["red"] * max_counts["green"]
        result_sum += curr_value
    return result_sum
...
if __name__ == "__main__":
    ...
    data = open(get_data(2), "r").read().split("\n")
