...
def part1(puzzle_input):
    times = [int(curr) for curr in puzzle_input[0].split()[1:]]
    distances = [int(curr) for curr in puzzle_input[1].split()[1:]]
    ways_to_win = []
    for time, distance in zip(times, distances):
        ...
        for i in range(1, time):
            if (time - i) * i > distance:
                ...
                break
        max_duration = 0
        for i in range(time, min_duration - 1, -1):
            if (time - i) * i > distance:
                max_duration = i
                break
        ways_to_win.append(max_duration - min_duration + 1)
    final_prod = 1
    for num in ways_to_win:
        final_prod = final_prod * num
    return final_prod


def part2(puzzle_input):
    time = int("".join([curr for curr in puzzle_input[0].split()[1:]]))
    distance = int("".join([curr for curr in puzzle_input[1].split()[1:]]))
    min_duration = 0
    for i in range(1, time):
        if (time - i) * i > distance:
            min_duration = i
