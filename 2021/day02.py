...
data = open(get_data(2)).read().split('\n')
...
def get_final_position(course_data):
    ...
    for line in course_data:
        ...
        if command == "forward":
            ...
            depth += units
    print(f"After following these instructions, you would have a horizontal position of {horizontal} and a depth of "
          f"{depth}. (Multiplying these together produces {horizontal * depth}.)")


def get_final_position_with_aim(course_data):
...
get_final_position(data)
get_final_position_with_aim(data)
