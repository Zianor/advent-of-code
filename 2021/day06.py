...
data = [int(number) for number in open(get_data(6)).read().split(',')]
...
After  2 days: 1,2,1,6,0,8              10
After  3 days: 0,1,0,5,6,7,8            11
After  4 days: 6,0,6,4,5,6,7,8,8        12
After  5 days: 5,6,5,3,4,5,6,7,7,8      13
After  6 days: 4,5,4,2,3,4,5,6,6,7      14
After  7 days: 3,4,3,1,2,3,4,5,5,6      15 
After  8 days: 2,3,2,0,1,2,3,4,4,5      16
After  9 days: 1,2,1,6,0,1,2,3,3,4,8    17
After 10 days: 0,1,0,5,6,0,1,2,2,3,7,8                          18
After 11 days: 6,0,6,4,5,6,0,1,1,2,6,7,8,8,8                    19
After 12 days: 5,6,5,3,4,5,6,0,0,1,5,6,7,7,7,8,8                20
After 13 days: 4,5,4,2,3,4,5,6,6,0,4,5,6,6,6,7,7,8,8            21
After 14 days: 3,4,3,1,2,3,4,5,5,6,3,4,5,5,5,6,6,7,7,8          22
After 15 days: 2,3,2,0,1,2,3,4,4,5,2,3,4,4,4,5,5,6,6,7          23
After 16 days: 1,2,1,6,0,1,2,3,3,4,1,2,3,3,3,4,4,5,5,6,8        24
After 17 days: 0,1,0,5,6,0,1,2,2,3,0,1,2,2,2,3,3,4,4,5,7,8
After 18 days: 6,0,6,4,5,6,0,1,1,2,6,0,1,1,1,2,2,3,3,4,6,7,8,8,8,8

After 19 days: 5,6,5,3,4,5,6,0,0,1,5,6,0,0,0,1,1,2,2,3,5,6,7,7,7,7,8,8
After 20 days: 4,5,4,2,3,4,5,6,6,0,4,5,6,6,6,0,0,1,1,2,4,5,6,6,6,6,7,7,8,8,8,8,8
...
def get_lanternfish_count(population, days):
    dict_of_fish_with_reproduction_number = {i: population.count(i) for i in population}
    for day in range(days):
        reproducing = dict_of_fish_with_reproduction_number.get(0, 0)
        new_dict = {}
        for i, count in dict_of_fish_with_reproduction_number.items():
            if i != 0:
                new_dict[i - 1] = count
        new_dict[8] = reproducing
        new_dict[6] = new_dict.get(6, 0) + reproducing
        dict_of_fish_with_reproduction_number = new_dict
    count = sum(dict_of_fish_with_reproduction_number.values())
    print(f"After {days} days, there would be a total of {count} lanternfish")


get_lanternfish_count(data, 80)
get_lanternfish_count(data, 256)
