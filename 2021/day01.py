...
data = [int(i) for i in open(get_data(1)).read().split('\n')]
...
def get_count_of_increasing_measurement(measurements):
    count = sum([1 if data[i] > data[i - 1] else 0 for i in range(1, len(measurements))])
    print(f"There are {count} measurements that are larger than the previous measurement")


def get_count_of_increasing_moving_window_sum(measurements, window_size=3):
    count = sum([1 if sum(data[i + 1:i + window_size + 1]) > sum(data[i:i + window_size]) else 0 for i in
                 range(0, len(measurements) - window_size + 1)])
    print(f"There are {count} sums that are larger than the previous sum.")


get_count_of_increasing_measurement(data)
get_count_of_increasing_moving_window_sum(data)
