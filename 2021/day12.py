...
data = open(get_data(12)).read().split('\n')
"""
...
def create_graph_dictonary(edges):
...
def get_number_of_paths(edges):
    graph = create_graph_dictonary(edges)
    paths = get_paths(graph, ["start"])
    print(f"There are {len(paths)} paths.")
    return len(paths)


def get_paths(graph, path):
    paths = []
    current_node = path[-1]
    if current_node == "end":
        return [path]
    for node in graph[current_node]:
        if node.isupper() or node not in path:
            new_paths = get_paths(graph, path + [node])
            for new_path in new_paths:
                paths.append(new_path)
    if len(paths) != 0:
        return [curr_path for curr_path in paths if curr_path != []]
    else:
        return []


def get_number_of_paths_part2(edges):
    graph = create_graph_dictonary(edges)
    paths = get_paths_part2(graph, ["start"])
    print(f"There are {len(paths)} paths.")
    return len(paths)


def get_paths_part2(graph, path):
    paths = []
    current_node = path[-1]
    if current_node == "end":
        return [path]
    small_caves = [node for node in path if node.islower()]
    small_caves_twice = True if len(small_caves) > len(set(small_caves)) else False
    for node in graph[current_node]:
        if node.isupper() or node not in path or (not small_caves_twice and node not in ["start", "end"]):
            new_paths = get_paths_part2(graph, path + [node])
            for new_path in new_paths:
                paths.append(new_path)
    if len(paths) != 0:
        return [curr_path for curr_path in paths if curr_path != []]
...
assert get_number_of_paths(example1) == 10
assert get_number_of_paths(example2) == 19

get_number_of_paths(data)

assert get_number_of_paths_part2(example1) == 36
assert get_number_of_paths_part2(example2) == 103

get_number_of_paths_part2(data)
