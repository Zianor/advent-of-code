...
data = open(get_data(10)).read().split('\n')
...
def get_syntax_error_lines_score(navigation_subsystem):
    delimiters = ['()', '[]', '{}', '<>']
    score = 0
    scores = {
        ')': 3,
    ...
    for line in navigation_subsystem:
        while any(delimiter in line for delimiter in delimiters):
...
def get_incomplete_lines_score(navigation_subsystem):
    delimiters = ['()', '[]', '{}', '<>']
    line_scores = []
    closing_delimiters = [')', ']', '}', '>']
    scores = {
    ...
    for line in navigation_subsystem:
        while any(delimiter in line for delimiter in delimiters):
            for delimiter in delimiters:
                line = line.replace(delimiter, '')
        if not any(closing_delimiter in set(line) for closing_delimiter in closing_delimiters):
            line_score = 0
            for char in reversed(line):
                line_score = line_score * 5 + scores[char]
            line_scores.append(line_score)
    score = median(line_scores)
    print(f"The total score for incomplete lines is {score}")
    return score
...
assert get_syntax_error_lines_score(example) == 26397

get_syntax_error_lines_score(data)

assert get_incomplete_lines_score(example) == 288957

get_incomplete_lines_score(data)
