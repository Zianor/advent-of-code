...
data = [int(number) for number in open(get_data(7)).read().split(',')]
...
def calculate_fuel_for_alignment_part1(crab_positions):
    point_median = round(median(crab_positions))
    fuel = sum([abs(point_median - crab_position) for crab_position in crab_positions])
    print(f"They need to spent {fuel} fuel to reach point {point_median}, the point which needs the least fuel")


def calculate_fuel_for_alignment_part2(crab_positions):
    point_floor = floor(sum(crab_positions) / len(crab_positions))
    point_ceil = ceil(sum(crab_positions) / len(crab_positions))
    fuel_floor = sum(
        [sum([i for i in range(1, abs(point_floor - crab_position) + 1)]) for crab_position in crab_positions])
    fuel_ceil = sum(
        [sum([i for i in range(1, abs(point_ceil - crab_position) + 1)]) for crab_position in crab_positions])
    fuel = min([fuel_ceil, fuel_floor])
    point = point_floor if fuel == fuel_floor else point_ceil
    print(f"They need to spent {fuel} fuel to reach point {point}, the point which needs the least fuel")


calculate_fuel_for_alignment_part1(data)
calculate_fuel_for_alignment_part2(data)
