...
data = np.array([list(line) for line in open(get_data(11), 'r').read().split('\n')], dtype=np.int64)
"""
...
def count_flashes_in_100_steps(energy_levels):
    flashes = 0
    for step in range(0, 100):
        energy_levels, new_flashes = calc_step(energy_levels)
        flashes += new_flashes
    print(f"After step {step + 1} there where {flashes} flashes")
    return flashes


def calc_step(energy_levels):
    energy_levels += 1
    flashes = 0
    flashed_this_step = np.zeros(energy_levels.shape)
    while energy_levels[np.logical_and(energy_levels > 9, flashed_this_step == 0)].size > 0:
        for i in range(energy_levels.shape[0]):
            for j in range(energy_levels.shape[1]):
                if energy_levels[i][j] > 9 and flashed_this_step[i][j] == 0:
                    flashes += 1
                    flashed_this_step[i][j] += 1
                    for i_flash in range(max(0, i - 1), min(energy_levels.shape[0], i + 2)):
                        for j_flash in range(max(0, j - 1), min(energy_levels.shape[1], j + 2)):
                            energy_levels[i_flash][j_flash] += 1
    energy_levels[energy_levels > 9] = 0
    return energy_levels, flashes


def first_synchronous_flash(energy_levels):
    step = 0
    while energy_levels[energy_levels != 0].size > 0:
        step += 1
        energy_levels, _ = calc_step(energy_levels)
    print(f"The first time they flash simultaneously is step {step}")
    return step


example_data = np.array([[5, 4, 8, 3, 1, 4, 3, 2, 2, 3],
                         [2, 7, 4, 5, 8, 5, 4, 7, 1, 1],
                         [5, 2, 6, 4, 5, 5, 6, 1, 7, 3],
                         [6, 1, 4, 1, 3, 3, 6, 1, 4, 6],
                         [6, 3, 5, 7, 3, 8, 5, 4, 7, 8],
...
assert count_flashes_in_100_steps(np.copy(example_data))

count_flashes_in_100_steps(np.copy(data))

assert first_synchronous_flash(np.copy(example_data)) == 195

first_synchronous_flash(np.copy(data))
