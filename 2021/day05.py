...
data = open(get_data(5)).read().split('\n')
...
def get_points_with_at_least_2_overlaps(vents):
    ...
    for vent in vents:
        ...
        if x1 == x2:
            ...
            for i in range(x1, x2 + direction, direction):
                overlaps[i, y1] += 1
    count = len([(x, y) for (x, y), value in overlaps.items() if value > 1])
    print(f"There are {count} points where least two lines overlap")


def get_points_with_at_least_2_overlaps_with_diagonals(vents):
    overlaps = defaultdict(int)
    for vent in vents:
        start, end = vent.split(' -> ')
        x1, y1 = start.split(',')
        ...
        if x1 == x2:
            for i in range(y1, y2 + y_direction, y_direction):
                overlaps[x1, i] += 1
        elif y1 == y2:
            ...
            for i in range(0, abs(y1 - y2) + 1):
                overlaps[x1 + i * x_direction, y1 + i * y_direction] += 1
    count = len([(x, y) for (x, y), value in overlaps.items() if value > 1])
    print(f"There are {count} points where least two lines overlap")


get_points_with_at_least_2_overlaps(data)
get_points_with_at_least_2_overlaps_with_diagonals(data)
