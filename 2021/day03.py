...
data = open(get_data(3)).read().split('\n')
...
def get_power_consumption(diagnostic_report):
    matrix = np.array([[int(bit) for bit in list(line)] for line in diagnostic_report])
    gamma_binary = ''.join(['1' if sum(matrix[:, i]) > matrix.shape[0] / 2 else '0' for i in range(matrix.shape[1])])
    epsilon_binary = ''.join(['1' if bit == '0' else '0' for bit in list(gamma_binary)])
    print(f"Multiplying the gamma rate ({int(gamma_binary, 2)}) by the epsilon rate ({int(epsilon_binary, 2)}) produces"
          f" the power consumption, {int(gamma_binary, 2) * int(epsilon_binary, 2)}.")


def get_matrix(diagnostic_report):
    return np.array([[int(bit) for bit in list(line)] for line in diagnostic_report])


def verify_life_support_rating(diagnostic_report):
    oxygen_generator_rating_binary = get_candidates(diagnostic_report, 0, '1')
    co2_scrubber_rating_binary = get_candidates(diagnostic_report, 0, '0')
    print(f"The life support rating produced by multiplying the oxygen generator rating "
          f"({int(oxygen_generator_rating_binary, 2)}) by the CO2 scrubber rating ({int(co2_scrubber_rating_binary, 2)}"
          f") is {int(oxygen_generator_rating_binary, 2) * int(co2_scrubber_rating_binary, 2)}.")


def get_candidates(candidates, bit_position, bit_equal):
    if len(candidates) == 1:
        return candidates[0]
    else:
        matrix = get_matrix(candidates)
        ratio = sum(matrix[:, bit_position]) >= matrix.shape[0] / 2
        if ratio >= 0.5:
            return get_candidates([candidate for candidate in candidates if candidate[bit_position] == bit_equal],
                                  bit_position + 1, bit_equal)
        return get_candidates([candidate for candidate in candidates if candidate[bit_position] != bit_equal],
                              bit_position + 1, bit_equal)


get_power_consumption(data)
verify_life_support_rating(data)
